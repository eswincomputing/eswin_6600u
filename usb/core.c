...
static void eswin_core_register_work(struct work_struct *work)
{
    ...
    if(dl_fw){
     	if (fw_name) {
    		ecrnx_printk_fw_dl("fw file name: %s\n",fw_name);
    	}
    	else {
	...
	if (dl_fw  && eswin_fw_file_chech(tr)) {
	    ecrnx_printk_fw_dl("%s entry, start fw download!!", __func__);
...
#endif
		dl_fw = false;
		ecrnx_printk_fw_dl("%s entry, finish and stop fw download!!", __func__);
		schedule_delayed_work(&tr->register_work, msecs_to_jiffies(1000));
		return;
	}

#ifdef CONFIG_ECRNX_WIFO_CAIL
	ecrnx_printk_fw_dl("%s entry, amt_mode = %d!!", __func__, amt_mode);
#endif
...
int usb_host_send(void *buff, int len, int flag)
{
	...
	ecrnx_printk_trans("usb_host_send, skb:0x%08x, skb_len:%d, frame_len:%d, flag:0x%08x \n", skb, skb->len, len, flag);

	if (tr->ops->xmit) {
		ret = tr->ops->xmit(tr, skb);
	} else {
		ecrnx_printk_err("eswin_sdio_work error, ops->xmit is null\n");
	}
