...
static int usb_host_tx_cfm_handler(struct ipc_host_env_tag *env, struct sk_buff *skb)
{
    ...
    ecrnx_printk_tx("%s:hostid(tx_skb):0x%08x, rx_skb: 0x%x \n", __func__, host_id, skb);
...
void usb_host_amt_rx_handler(uint32_t frm_type, struct sk_buff *skb)
{
    ...
    ecrnx_printk_amt("%s enter, frame type: %d!!", __func__, frm_type);
    if(!skb)
    {
        ecrnx_printk_err("usb_host_amt_rx_handler input param error!! \n!");
        return;
    }
    ...
    ecrnx_printk_amt("skb:0x%08x, skb_len:%d, frame type: %d!!", skb->data,skb->len, frm_type);
...
void usb_host_rx_handler(uint32_t frm_type, struct ipc_host_env_tag *env, struct sk_buff *skb)
{
    ...
    if(!env || !skb)
    {
        ecrnx_printk_err("usb_host_rx_handler input param error!! \n!");
        return;
    }
    ...
    switch (frm_type)
    {
        ...
        case USB_FRM_TYPE_MSG_ACK:
        {
            if(1 == skb->len)
            {
                ecrnx_printk_err("MSG_ACK len: 1");
                break;
            }
        ...
        case USB_FRM_TYPE_TXCFM:
        {
            if(!env->pthis)
            {
                ecrnx_printk_err("env->pthis ptr error!! \n!");
                break;
            }
        ...
        case USB_FRM_DEBUG_FS:
        {
            ...
            (debugfs_type < SLAVE_DEBUGFS_MAX)){
                ...
                ecrnx_printk_debugfs("%s - wake_up()\n", __func__);
    ...
    if (!ret && skb) { // free the skb
    	ecrnx_printk_rx("%s:skb free: 0x%x, ret: %d; \n", __func__, skb, ret);
        dev_kfree_skb(skb);
    }
