                                            ...
                                            struct ecrnx_sta *sta, struct station_parameters *params)
{
    ...
    if(!sta || !params)
    {
        ecrnx_printk_err("%s-%d:sta(%p) or params(%p) is null, return!\n", __func__, __LINE__, sta, params);
        return;
    }
    ...
    if (!debugfs_sta)
        ecrnx_printk_err("error debugfs_sta kzalloc!\n");
...
void argv_display(uint32_t argc, char* argv[])
{
    ...
    ecrnx_printk_always("argc is %d\n", argc);
    for (i=0; i<argc; i++)
    {
        ecrnx_printk_always("param %d is %s\n", i, argv[i]);
    }
}
...
int cli_check_parm_range(uint32_t start_index, uint32_t index, uint32_t argc, char* argv[])
{
    ...
    for (i=start_index; i<argc; i++)
    {
        ...
        {
            ecrnx_printk_always("param %d is out of range! current %d, min %d, max %d\n",
...
int cli_cmd_parse(uint8_t *cmd)
{
    ...
    ecrnx_printk_err("No matching commands!\n");
