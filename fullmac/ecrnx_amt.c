...
{
	ecrnx_printk_amt("amt netdev open\n");
...
{
	ecrnx_printk_amt("amt netdev stop\n");
...
{
    if (amt_conf_param.delta_gain_flag == 0)
        ecrnx_printk_always("parse amt.cfg delta gain fail\n");
    if (amt_conf_param.cfo_flag == 0)
        ecrnx_printk_always("parse amt.cfg cfo fail\n");
    if (amt_conf_param.mac_flag == 0)
        ecrnx_printk_always("parse amt.cfg mac fail\n");

    if (amt_conf_param.delta_gain_flag == 0 && amt_conf_param.cfo_flag == 0 && amt_conf_param.mac_flag == 0 )
        return -1;
    
    ecrnx_printk_always(" ecrnx_amt_config_msg_send success\n");

    host_send((void *)&amt_conf_param, sizeof(amt_conf_param), TX_FLAG_AMT_CONFIG_IE);
...
int ecrnx_amt_init(void *ecrnx_plat)
{
...
#endif
	if (!ndev) {
		ecrnx_printk_err("alloc netdev fail !!");
		return -1;
	}
	...
	ecrnx_printk_always(" %s:register the amt net device success\n", __func__);
	return 0;

err_dev:
	ecrnx_printk_err(" %s couldn't register the amt net device!!",  __func__);
	free_netdev(ndev);
	amt_vif.ndev = NULL;
    return -1;
}
...
{
    ecrnx_printk_amt(ECRNX_FN_ENTRY_STR);
