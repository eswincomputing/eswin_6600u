...
void reg_display_u32(u32 addr, u32 *reg, u32 len)
{
    ...
    {
        if (i%4 == 0)
            ecrnx_printk_always("0x%08X", (addr+ 4*i));

        ecrnx_printk_always("0x%08X", reg[i]);
...
void reg_display_u8(u32 addr, u8 *reg, u32 len)
{
    ...
    {
        if (i%16 == 0)
            ecrnx_printk_always("0x%04X  ", (addr+i)&0xFFFF);

        ecrnx_printk_always(" %02X", reg[i]);
...
static int log_level_show(struct seq_file *seq, void *v)
{
    ...
    if (0 == ecrnx_log_level_get(&log))
        seq_printf(seq, "host_printk_mask:0x%x log_level:%d  dir:%d\n", ecrnx_printk_mask, log.level, log.dir);
    else
        seq_printf(seq, "\n");
                            ...
                            size_t count, loff_t *ppos)
{
    ...
    ecrnx_printk_debug("log_level_write count:%d \n", count);
    if (count > 15)  // 255:0\n
        return -1;
    ...
    sscanf(kbuf, "%x %d %d\n",  &ecrnx_printk_mask, &level, &dir);
    ecrnx_printk_always("%s:host_dbg_level:0x%x, level:%d, dir:%d \n", __func__, ecrnx_printk_mask, level, dir);
                                   ...
                                   size_t count, loff_t *ppos)
{
    ...
    while(buf_handle->show)
    {
        ...
        len = min(len, 512);
        //ecrnx_printk_always("mac_log_read len:%d %d %d", len,buf_handle->write_point, buf_handle->read_point);
        if(len > 0)
        {
                            ...
                            size_t count, loff_t *ppos)
{
    ...
    if (ret_code != 2)
    {
        ecrnx_printk_err("Parameter error.\n");
        return -1;
    }

    if ( addr%4 )
    {
        ecrnx_printk_err("Address is invalid.\n");
        return -1;
    }
...
static int sta_info_in_ap_show(struct seq_file *seq, void *v)
{
	...
	if (seq->private == NULL)
		ecrnx_printk_err("error sta_info_in_ap_show\n");
...
void ecrnx_debugfs_sta_in_ap_init(void *private_data)
{
	...
	ecrnx_printk_always("%s:file_name: %s, sta idx:%d,stas:0x%p \n", __func__, file_name, sta->sta_idx, stas[index]);
}
...
void ecrnx_debugfs_sta_in_ap_del(u8 sta_idx)
{
    ...
    ecrnx_printk_always("%s: sta_idx:%d, stas:0x%p \n", __func__, sta_idx, stas[sta_idx]);
    dentry_sta = stas[sta_idx];
    if(!dentry_sta)
    {
        ecrnx_printk_err("error sta_idx!!!\n");
        return;
    }
    ...
    if (dentry_sta->d_inode)
    {
        ...
        ecrnx_printk_debugfs("%s: dentry_sta->d_inode:0x%p, debugfs_sta:0x%p \n", __func__, dentry_sta->d_inode, debugfs_sta);
        if (debugfs_sta)
        {
