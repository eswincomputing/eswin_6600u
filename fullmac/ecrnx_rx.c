                             ...
                             struct sk_buff *skb,  struct hw_rxhdr *rxhdr)
{
    ...
    if (amsdu) {
        ...
        if (count == 0)
        {
            ecrnx_printk_err("amsdu decode fail!!\n");
            //return false;
    ...
    if (((ECRNX_VIF_TYPE(ecrnx_vif) == NL80211_IFTYPE_AP) ||
        ...
        if (!rx_skb)
        {
            ecrnx_printk_err("rx skb is null amsdu:%d skb->len=%d\n", amsdu, skb->len);
            return true;
        }
        ...
        if (!eth)
        {
            ecrnx_printk_err("eth is null amsdu:%d skb->len=%d\n", amsdu, skb->len);
            return true;
        }
    ...
    while (!skb_queue_empty(&list)) {
        ...
        if (!rx_skb)
        {
            ecrnx_printk_err("rx_skb is null\n");
            return true;
        }
        ...
        if (forward) {
            ...
            ecrnx_printk_rx("netif_receive_skb, skb:%p, skb_len:%d; \n", rx_skb, rx_skb->len);
                           ...
                           u8 rtap_len)
{
    ...
    ecrnx_printk_rx("%s enter!!", __func__);
    ...
    ecrnx_printk_rx("%s exit!!", __func__);
    return 0;
}
...
int ecrnx_rx_reord_single_msdu(struct ecrnx_hw *ecrnx_hw, struct ecrnx_vif *ecrnx_vif, struct reord_msdu_info *pmsdu)
{
    ...
    if (pmsdu->need_pn_check) {
        if (ecrnx_rx_data_pn_check(ecrnx_hw, skb,  pmsdu->hw_rxhdr->flags_vif_idx, pmsdu->hw_rxhdr->flags_sta_idx, pmsdu->tid, pmsdu->is_ga)){
            ...
            ecrnx_printk_rx("rx_data_check_pn error\n");
            dev_kfree_skb(skb);
            goto end;
        }
    }
    ...
    {
        ecrnx_printk_debug("miss[%d] last:%d sn=%d\n",pmsdu->tid,last_sn[pmsdu->tid], pmsdu->sn);

        if(pmsdu->hw_rxhdr->flags_sta_idx != ECRNX_INVALID_STA)
        {
            int i;
            struct reord_cntrl *reord_cntrl = NULL;

            reord_cntrl = &ecrnx_hw->sta_table[pmsdu->hw_rxhdr->flags_sta_idx].reord_cntrl[pmsdu->tid];
            for(i = last_sn[pmsdu->tid]+1; i < pmsdu->sn; i++)
            {
                reord_cntrl->skip_sn[reord_cntrl->skip_idx++] = i;
                reord_cntrl->skip_idx =  reord_cntrl->skip_idx % SKIP_SN_LIST_CNT;
            }
        }
    }
...
void ecrnx_rx_reord_check(struct ecrnx_hw *ecrnx_hw, struct ecrnx_vif *ecrnx_vif, struct hw_rxhdr *hw_rxhdr, struct sk_buff *skb, struct rxu_stat_mm* orignal_rxu_state)
{
    ...
    if (!pmsdu) {
        ecrnx_printk_err("ecrnx_rx_reord_msdu_alloc fail\n");
        dev_kfree_skb(skb);
        return ;
    }
    ...
    if (hw_rxhdr->flags_sta_idx != ECRNX_INVALID_STA) {
        ...
        if(reord_cntrl->valid) {
            ...
            ecrnx_printk_warn("reord_cntrl invalid sta:%d sn:%d start:%d \n", hw_rxhdr->flags_sta_idx , pmsdu->sn, reord_cntrl->win_start);
            ecrnx_rx_reord_single_msdu(ecrnx_hw, ecrnx_vif, pmsdu);
            return ;
        }
    } else {
        ecrnx_printk_warn("sta_idx invalid sta:%d sn:%d start:%d  \n", hw_rxhdr->flags_sta_idx , pmsdu->sn, reord_cntrl->win_start);
        ecrnx_rx_reord_single_msdu(ecrnx_hw, ecrnx_vif, pmsdu);
        return ;
    }
    ...
    if (ecrnx_rx_reord_sn_check(reord_cntrl, pmsdu->sn)) {
        ...
        spin_unlock(&reord_cntrl->reord_list_lock);
        int i;
        for(i=0; i < SKIP_SN_LIST_CNT; i++)
        {
            if(reord_cntrl->skip_sn[i] == pmsdu->sn)
            {
                reord_cntrl->skip_sn[i] = 0xffff;
                ecrnx_printk_debug("%s timeout discard sn:%d s:%d \n", __func__, pmsdu->sn, reord_cntrl->win_start);
                break;
            }
        }
		
        ecrnx_rx_fast_update_msdu(ecrnx_hw, ecrnx_vif, pmsdu);
        return ;
        //goto discard;
    }
    //printk("start:%d %d\n",reord_cntrl->win_start, pmsdu->sn);
        ...
        if (hw_rxhdr->flags_is_80211_mpdu) {
            ...
            if (!ecrnx_vif) {
                dev_err(ecrnx_hw->dev, "Frame received but no active vif (%d)",
                        hw_rxhdr->flags_vif_idx);
                dev_kfree_skb(skb);
				ecrnx_printk_err("[agg] check_alloc, %d !!", __LINE__);
                goto exit_no_free;
            }
...
u8 ecrnx_rxdataind(void *pthis, void *hostid)
{
    ...
    if(!pthis || !hostid){
        ecrnx_printk_err("ecrnx_rxdataind error! \n");
        return 0;
    }
    ...
    if(orignal_rxu_state->fragment_flag && ((orignal_rxu_state->status & RX_STAT_FORWARD) || (orignal_rxu_state->status & RX_STAT_DELETE))) { //defrag frame
        ...
        if(list_empty(&ecrnx_hw->defrag_rx_list)){
            ecrnx_printk_err("error defrag_rx_list is epmty!!! \n");
            dev_kfree_skb(skb);
            goto end;
        }

        ecrnx_printk_agg("[eswin_agg]%s enter, tid:0x%x, fragment_flag:0x%llx, status:0x%x \n", __func__, orignal_rxu_state->tid, orignal_rxu_state->fragment_flag, orignal_rxu_state->status);
        if(orignal_rxu_state->status & RX_STAT_DELETE){
            ...
            list_for_each_entry_safe(defrag_elem_rx, next, &ecrnx_hw->defrag_rx_list, head) {
                ecrnx_printk_agg("[eswin_agg]forward: sn1:%d, sn2:%d, tid1:%d, skb:0x%p \n",defrag_elem_rx->sn, orignal_rxu_state->sn, defrag_elem_rx->tid, defrag_elem_rx->skb);
                if((defrag_elem_rx->tid == orignal_rxu_state->tid) && (defrag_elem_rx->sn == orignal_rxu_state->sn)){
                    if (fg_first) {
					    offset_len = 0;
					    orignal_rxu_state->need_pn_check = defrag_elem_rx->need_pn_check;
                    } else {
                        /*first skb should include the hw_rxhdr, other's not need*/
                        offset_len = hw_rxhdr_len;
                        offset_len += sizeof(struct ethhdr);
                        if(defrag_elem_rx->need_pn_check) {
                            ecrnx_printk_agg("need pn check, skb:%p \n", defrag_elem_rx->skb);
                            offset_len +=8; //pn offset
                        }
                    }
    ...
    if (hw_rxhdr->pattern != ecrnx_rxbuff_pattern) {
        dev_err(ecrnx_hw->dev, "RX Buff Pattern not correct, pattern (%x), status %d\n", hw_rxhdr->pattern, status);
        ecrnx_printk_err("RX Buff Pattern not correct, pattern (%x), status %d, skb (%p), skb_len %d\n", hw_rxhdr->pattern, status, skb->data, skb->len);
        print_hex_dump(KERN_DEBUG, DBG_PREFIX_PAT, DUMP_PREFIX_NONE, 16, 1, hw_rxhdr, skb->len, false);
        if(skb){
            dev_kfree_skb(skb);
        }
...
check_len_update:
    ...
    if (status & RX_STAT_SPURIOUS) {
        ...
        ecrnx_vif = ecrnx_rx_get_vif(ecrnx_hw, hw_rxhdr->flags_vif_idx);
        ecrnx_printk_rx(" flags vif idx:%d, vif: 0x%x \n", hw_rxhdr->flags_vif_idx, ecrnx_vif);
        if (ecrnx_vif) {
        ...
        if (hw_rxhdr->flags_is_80211_mpdu) {
            ...
            if (!ecrnx_vif) {
                dev_err(ecrnx_hw->dev, "Frame received but no active vif (%d)",
                        hw_rxhdr->flags_vif_idx);
                dev_kfree_skb(skb);
				ecrnx_printk_err(" check_alloc, %d !!", __LINE__);
                goto end;
            }
        ...
        /*agg frame*/
           ...
           if(orignal_rxu_state->fragment_flag){
            ...
            if(defrag_elem_rx){
                ...
                defrag_elem_rx->need_pn_check = orignal_rxu_state->need_pn_check;
                defrag_elem_rx->sn = orignal_rxu_state->sn;
                defrag_elem_rx->real_offset = orignal_rxu_state->real_offset;
                defrag_elem_rx->skb = skb;
                ecrnx_printk_rx("ecrnx_rxdataind:insert_skb:0x%08x, skb_len:%d, tid:%d, sn:%d, need_pn_check:%d \n", skb, skb->len, orignal_rxu_state->tid, orignal_rxu_state->sn, orignal_rxu_state->need_pn_check);
                list_add_tail(&defrag_elem_rx->head, &ecrnx_hw->defrag_rx_list);
            }else{
                ecrnx_printk_err("no buffer !!!! \n");
            }
        }
...
int ecrnx_data_cfm_callback(void *priv, void *host_id)
{
    ...
    {
        ecrnx_printk_err("ecrnx_data_cfm_callback input param error!! \n!");
        return -1;
    }
...
int ecrnx_msg_cfm_callback(void *priv, void *host_id)
{
    ...
    if(!env || !env->pthis || host_id == 0)
    {
        ecrnx_printk_err("ecrnx_msg_cfm_callback input param error!! \n!");
        return -1;
    }
...
#endif
{
    ...
    if(!skb || !skb->data)
    {
        ecrnx_printk_err("sdio rx err error \n");
    }
    ...
    {
        ecrnx_printk_err("endopint error \n");
    }
#endif
