...
static int ecrnx_open(struct net_device *dev)
{
    ...
    if (ECRNX_VIF_TYPE(ecrnx_vif) == NL80211_IFTYPE_AP_VLAN) {
        ...
        if (add_if_cfm.status != 0) {
            ecrnx_printk_err("%s: Status Error(%d)\n", __func__, add_if_cfm.status);
            return -EIO;
        }
    }
...
static int ecrnx_close(struct net_device *dev)
{
    ...
    ecrnx_printk_cfg(ECRNX_FN_ENTRY_STR);
                                               ...
                                               struct vif_params *params)
{
...
#endif
    {
        ...
        else
        {
            if(vif_idx !=0)
            {
                if(ndev->dev_addr[0] & 0x02)
                {
                  ndev->dev_addr[0] = ((ndev->dev_addr[0] >> 2) ^ (vif_idx))<< 2;
                  ndev->dev_addr[0] |= 0x02;
                }
                else
                {
                  ndev->dev_addr[0] = ((ndev->dev_addr[0] >> 2) ^ (vif_idx-1))<< 2;
                  ndev->dev_addr[0] |= 0x02;
                }
            }
        }
...
static int ecrnx_cfg80211_del_iface(struct wiphy *wiphy, struct wireless_dev *wdev)
{
    ...
    if (dev->reg_state == NETREG_REGISTERED) {
      ecrnx_printk_cfg("%s-%d:unregister_netdevice \n", __func__, __LINE__);
                                      ...
                                      struct vif_params *params)
{
    ...
    ecrnx_printk_cfg("%s:dev:0x%p, type:%d, vif->up:%d \n", __func__, dev, type, vif->up);
    ...
    if (vif->up)
    {
        ...
        {
            ecrnx_send_disconnect_local(ecrnx_hw, vif, ECRNX_WLAN_STATUS_ASSOC_UNSPEC);
        }
        ecrnx_printk_warn("ecrnx_cfg80211_change_iface: -EBUSY \n");
        ecrnx_close(dev);
    }
...
static int ecrnx_cfg80211_start_p2p_device(struct wiphy *wiphy,
				      struct wireless_dev *wdev)
{
	ecrnx_printk_cfg("rwnx_cfg80211_start_p2p_device\n");
	return 0;
}
                              ...
                              struct cfg80211_scan_request *request)
{
    ...
    if(!ecrnx_hw->scan_request){
        ...
        ecrnx_printk_cfg("%s:scan_request:0x%p \n", __func__, request);
        if ((error = ecrnx_send_scanu_req(ecrnx_hw, ecrnx_vif, request))){
            ecrnx_printk_err("scan message send error!!\n");
            ecrnx_hw->scan_request = NULL;
            return error;	
    	}
    }else{
        ecrnx_printk_warn("scan is already running!!\n");
    }
    ecrnx_printk_cfg("send finish:ecrnx_cfg80211_scan \n");
                              ...
                              struct wireless_dev *wdev)
{
    ...
    ecrnx_printk_cfg("%s:ecrnx_hw->scan_request:0x%p \n", __func__, ecrnx_hw->scan_request);

    if(!ecrnx_hw->scan_request){
        ecrnx_printk_err("no scan is running, don't need abort! \n");
        goto out;
    }

    if(wdev->iftype != NL80211_IFTYPE_STATION){
        ecrnx_printk_err("abort scan ignored, iftype(%d)\n", wdev->iftype);
        goto out;
    }

    if(wdev != ecrnx_hw->scan_request->wdev){
        ecrnx_printk_err("abort scan was called on the wrong iface\n");
        goto out;
    }
                                 ...
                                 struct cfg80211_connect_params *sme)
{
    ...
    if(ecrnx_vif) {
        if(ecrnx_vif->sta.ap && ecrnx_vif->wdev.iftype == NL80211_IFTYPE_STATION) {
            if(ecrnx_vif->sta.ap->valid) {
                ecrnx_printk_warn("already connected, disconnect first; \n");
                ecrnx_send_disconnect_local(ecrnx_hw, ecrnx_vif, ECRNX_WLAN_STATUS_ASSOC_UNSPEC);
            }
        }
    }
    ...
    if ((error = ecrnx_send_sm_connect_req(ecrnx_hw, ecrnx_vif, sme, &sm_connect_cfm)))
        return error;
    ecrnx_printk_cfg("%s:bssid:%pM, send status:%d\n", __func__, sme->bssid, sm_connect_cfm.status);
                                    ...
                                    u16 reason_code)
{
    ...
    ecrnx_printk_cfg("%s:dev:0x%p, vif_index:%d, reason_code:%d \n", __func__, dev, ecrnx_vif->vif_index, reason_code);
        ...
        case CO_OK:
        {
            ...
            if((ECRNX_VIF_TYPE(ecrnx_vif) == NL80211_IFTYPE_AP) || (ECRNX_VIF_TYPE(ecrnx_vif) == NL80211_IFTYPE_P2P_GO)) {
                ...
                if((!is_multicast_sta(sta->sta_idx)) && (sta->mac_addr)) {
                    ...
                    ecrnx_printk_cfg("%s-%d:sta:0x%x,sta->mac_addr:%pM \n", __func__, __LINE__, sta, sta->mac_addr);
...
)
{
    ...
    list_for_each_entry_safe(cur, tmp, &ecrnx_vif->ap.sta_list, list) {
        if ((!mac) || (!memcmp(cur->mac_addr, mac, ETH_ALEN))) {
            ...
            ecrnx_printk_cfg("%s-%d:cur_list:0x%p, vif_list:0x%p, mac:%pM\n", __func__, __LINE__, &cur->list, &ecrnx_vif->ap.sta_list, mac);
            ...
            if ((error != 0) && (error != -EPIPE)){
                ecrnx_printk_warn("del sta msg send fail, error code:%d \n", error);
            }
            ...
            if((ECRNX_VIF_TYPE(ecrnx_vif) == NL80211_IFTYPE_AP) || (ECRNX_VIF_TYPE(ecrnx_vif) == NL80211_IFTYPE_P2P_GO)) {
                ...
                if((!is_multicast_sta(cur->sta_idx)) && params) {
                    if(params->mac){
                        ecrnx_printk_cfg("%s-%d:vif:%d, mac:%pM \n", __func__, __LINE__, ECRNX_VIF_TYPE(ecrnx_vif), params->mac);
                        cfg80211_del_sta(ecrnx_vif->ndev, params->mac, GFP_ATOMIC);
                    }
                }
#else
            if((!is_multicast_sta(cur->sta_idx)) && mac) {
                    if(mac){
                        ecrnx_printk_cfg("%s-%d:vif:%d, mac:%pM \n", __func__, __LINE__, ECRNX_VIF_TYPE(ecrnx_vif), mac);
                        cfg80211_del_sta(ecrnx_vif->ndev, mac, GFP_ATOMIC);
                    }
                }
#endif
            }
                                ...
                                unsigned int duration, u64 *cookie)
{
    ...
    if (ecrnx_hw->roc)
    {
        ecrnx_printk_err("%s-%d,statu error!!!, duration:%d \n", __func__, __LINE__, duration);
        //exp_report(ecrnx_hw);
		...
		if (timer)
			ecrnx_printk_cfg("wait_event: wake up!!! timer:%ld \n", timer);
		else
			ecrnx_printk_cfg("wait_event: timout!!!\n");
#endif
        //return -EBUSY;
    }
        ...
        *cookie = (u64)(ecrnx_hw->roc_cookie);
		...
		if(ecrnx_vif->mgmt_reg_stypes & BIT(IEEE80211_STYPE_PROBE_REQ >> 4))
		{
			if(ecrnx_send_p2p_start_listen_req(ecrnx_hw, ecrnx_vif, duration))
				ecrnx_printk_err("P2P: start_listen failed\n");
		}
#endif
    } else {
        kfree(roc);
                                                  ...
                                                  u64 cookie)
{
	...
	//if(ecrnx_vif->mgmt_reg_stypes & BIT(IEEE80211_STYPE_PROBE_REQ >> 4))
	{
		...
		if(error == 0)
			ecrnx_printk_cfg("P2P: cancel_listen OK!!!\n");
		else
			ecrnx_printk_err("P2P: cancel_listen failed, error=%d\n", error);
	}
#endif
    /* Forward the information to the FMAC */
                        ...
                        bool initiator, const u8 *buf, size_t len)
{
    ...
    if ((action_code == WLAN_TDLS_SETUP_REQUEST) &&
        ...
        (ecrnx_vif->tdls_status == TDLS_LINK_ACTIVE)) {
        ecrnx_printk_err("%s: only one TDLS link is supported!\n", __func__);
        return -ENOTSUPP;
    }

    if ((action_code == WLAN_TDLS_DISCOVERY_REQUEST) &&
        (ecrnx_hw->mod_params->ps_on)) {
        ecrnx_printk_err("%s: discovery request is not supported when "
    ...
    default:
        ecrnx_printk_err("%s: Unknown TDLS mgmt/action frame %pM\n",
                __func__, peer);
        ret = -EOPNOTSUPP;
        break;
    }
        ...
        const u8 *peer, enum nl80211_tdls_operation oper)
{
    ...
    if (!ecrnx_vif->sta.tdls_sta) {
        ecrnx_printk_err("%s: TDLS station %pM does not exist\n", __func__, peer);
        return -ENOLINK;
    }
                                      ...
                                      struct cfg80211_chan_def *chandef)
{
    ...
    if ((!ecrnx_sta) || (memcmp(addr, ecrnx_sta->mac_addr, ETH_ALEN))) {
        ecrnx_printk_err("%s: TDLS station %pM doesn't exist\n", __func__, addr);
        return -ENOLINK;
    }

    if (!ecrnx_sta->tdls.chsw_allowed) {
        ecrnx_printk_err("%s: TDLS station %pM does not support TDLS channel switch\n", __func__, addr);
        return -ENOTSUPP;
    }
    ...
    if (!cfm.status) {
        ...
        ecrnx_printk_err("%s: TDLS channel switch already enabled and only one is supported\n", __func__);
        return -EALREADY;
    }
}
...
int ecrnx_cfg80211_init(void *ecrnx_plat, void **platform_data)
{
    ...
    *platform_data = ecrnx_hw;
    ...
    {//parse amt.cfg         
        struct device *dev;        
        dev = ecrnx_platform_get_dev(ecrnx_plat);        
        ecrnx_parse_amt_configfile(dev, ECRNX_CONFIG_AMT_NAME);        
	...
	if ((ret = ecrnx_get_cal_result(ecrnx_hw))) {
		...
		if ((!mac_flag) && (0 == (cal_result.mac_addr[0] & 0x1)) && (cal_result.mac_addr[0] || cal_result.mac_addr[1] 
...
err_cache:
    wiphy_free(wiphy);
err_out:
    ecrnx_printk_err(" %s cfg80211 init failed %d!!", __func__, ret);
    return ret;
}
...
void ecrnx_cfg80211_deinit(struct ecrnx_hw *ecrnx_hw)
{
    ...
    if(ecrnx_hw->wiphy)
    {
        ecrnx_printk_cfg("%s wiphy_unregister \n", __func__);
        wiphy_unregister(ecrnx_hw->wiphy);
