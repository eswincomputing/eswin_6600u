...
struct sk_buff * sdio_rx_buf_push(struct sk_buff *skb, uint16_t recv_len)
{
    ...
    ecrnx_printk_trans("%s enter, skb: %d, skb_data:%d, skb_len:%d", __func__, skb, skb->data, recv_len);
    ...
    {
        ecrnx_printk_err("no enough space \n");
        return NULL;
    }

    if((!skb) || (!recv_len) || (recv_len > SDIO_RXBUF_SIZE))
    {
        ecrnx_printk_err("rx data is error \n");
        return NULL;
    }
...
struct sk_buff* sdio_rx_buf_pop(void)
{
    ...
    if(atomic_read(&sdio_rx_buff.suspend))
    {
    	ecrnx_printk_err("sdio suspend! \n");
        return NULL;
    }

    if(sdio_rx_buff.sdio_host_rx_buff_used <= 0)
    {
        ecrnx_printk_err("no data in memory \n");
        return NULL;
    }
...
void sdio_host_amt_rx_handler(uint32_t frm_type, struct sk_buff *skb)
{
    ...
    ecrnx_printk_amt("%s enter, frame type: %d, len %d.!!", __func__, frm_type, skb->len);
    ...
    if (need_free && skb) { // free the skb
		ecrnx_printk_amt("skb free: 0x%x, \n", skb);
        dev_kfree_skb(skb);
    }
    ecrnx_printk_amt("%s exit!!", __func__);
    return;
}
#endif
...
void sdio_host_rx_handler(uint32_t frm_type, struct ipc_host_env_tag *env, struct sk_buff *skb)
{
    ...
    ecrnx_printk_rx("%s enter, frame type: %d!!", __func__, frm_type);
    if (frm_type != SDIO_FRM_TYPE_RXDESC)
    {
        skb_pull(skb, SKB_DATA_COM_HD_OFFSET); //delete the frame common header
    }
    ...
    if (!ret && skb) { // free the skb
    	ecrnx_printk_rx("skb free: 0x%x, ret: %d!! \n", skb, ret);
        dev_kfree_skb(skb);
    }
...
{
    ecrnx_printk_trans(ECRNX_FN_ENTRY_STR);
