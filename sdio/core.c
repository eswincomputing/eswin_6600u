...
static void eswin_core_register_work(struct work_struct *work)
{
...
#ifdef CONFIG_ECRNX_WIFO_CAIL
	ecrnx_printk_always(" %s entry, amt_mode = %d!!", __func__, amt_mode);
#endif
    ...
    ecrnx_printk_exit("%s exit!!", __func__);
...
{
	ecrnx_printk_init("%s entry!!", __func__);
	tr->ops->start(tr);
...
static int eswin_sdio_tx_thread(void *data)
{
...
#endif
		ecrnx_printk_init("sdio pkg thread entry\n");
	...
	while (!kthread_should_stop())
	{
		...
		if (ret < 0)
		{
			ecrnx_printk_err("sdio pkg thread error!\n");
			return 0;
		}
		...
		while (tr->tx_pkg_queue.count != 0)
		{
			...
			if (tr->ops->xmit) {
				...
				ecrnx_printk_err(" eswin_sdio_work, ops->xmit is null\n");
			}
		}
	}
	ecrnx_printk_exit("sdio tx thread exit\n");
	return 0;
}
...
static int eswin_sdio_callback_thread(void *data)
{
	...
	ptr_addr host_id;
		...
		sched_set_fifo(get_current());
#endif
		ecrnx_printk_init("sdio callback thread entry\n");
	...
	while (!kthread_should_stop())
	{
		...
		if (ret < 0)
		{
			ecrnx_printk_err("sdio callback thread error!\n");
			return 0;
		}
		if(kthread_should_stop())
		{
	...
	ecrnx_printk_exit("rx callback thread exit\n");
	return 0;
}
...
static int eswin_sdio_tx_pkg_thread(void *data)
{
	...
	int i, ret, pkg_cnt = 0;
		...
		sched_set_fifo(get_current());
#endif
		ecrnx_printk_init("sdio tx pkg thread entry\n");
	...
	while (!kthread_should_stop())
	{
		...
		if (ret < 0)
		{
			ecrnx_printk_err("sdio tx pkg thread error!\n");
			return 0;
		}
		if(kthread_should_stop())
		{
		...
		while (tr->tx_queue.count != 0)
		{
			...
			if (!pkg_node) {
			    ecrnx_printk_err(" sdio pkg failed, no node!!\n");
			    break;
			}
			...
			else
			{
				...
				if(!pkg_node->buff){
			        ecrnx_printk_err("pkg_node buff malloc error! \n");
				}
	...
	ecrnx_printk_exit("tx pkg thread exit\n");
	return 0;
}
...
int sdio_host_send(void *buff, int len, int flag)
{
    ...
    ecrnx_printk_trans("%s enter, data len :%d ", __func__, len);

    if (!node) {
        ecrnx_printk_err(" sdio send failed, no node!!\n");
        return -1;
    }

    node->buff = (struct lmac_msg *)kzalloc(len, GFP_ATOMIC);
    if(!node->buff){
        ecrnx_printk_err("buff malloc error! \n");
    }
...
void eswin_core_destroy(struct eswin *tr)
{
    ...
    ecrnx_printk_always("%s node_num %d\n", __func__, tr->tx_node_num);
    spin_lock_irqsave(&tr->tx_lock,flags);
    for (i=0; i<64; i++) 
    {
