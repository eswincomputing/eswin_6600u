...
static int sdio_rx_param_init(void)
{
    ...
    {
        ecrnx_printk_trans("0x%x ", skb->data[index]);
    }

    ecrnx_printk_trans("%s exit, skb_len:%d, type: %d!!", __func__, skb->len, (skb->data[1] << 8) | skb->data[0]);
...
static void test_timer_handler(struct timer_list * lt)
{
    ecrnx_printk_trans("%s, counter:%d\n", __FUNCTION__, counter);
