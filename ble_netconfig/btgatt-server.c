...
static void update_wpa_network(void)
{
	...
	sprintf(sys_cmd, "sudo pkill -9 wpa_supplicant");
	result = wifi_run_cmd(sys_cmd);
	if(result < 0){
		return;
	}
					...
					void *user_data)
{
	...
	uint8_t error = 0;
	static int count=0;
	static uint8_t ssid_len_continue=0;
	ssid_len = 0;
	if (value&&(count==0))
	{
		memset(netcfg_ssid, 0x00, CFG_ITEM_LEN);
		memcpy(netcfg_ssid + offset, value, len);
		ssid_len_continue +=len;
		
	}
	if(count==1)
	{
		memcpy(netcfg_ssid + offset+ssid_len_continue, value, len);
		ssid_len_continue +=len;
		count=0;
		//PRLOG("ESWIN SSID write called offset=%d len=%d\n",offset, ssid_len_continue);	
		if (offset+ssid_len_continue >= CFG_ITEM_LEN)
		{
			error = BT_ATT_ERROR_INVALID_OFFSET;
			goto done;
		}
