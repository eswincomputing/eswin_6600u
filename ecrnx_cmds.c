...
static void cmd_dump(const struct ecrnx_cmd *cmd)
{
    ...
    ecrnx_printk_always("tkn[%d]  flags:%04x  result:%3d  cmd:%4d-%-24s - reqcfm(%4d-%-s)\n",
...
static int cmd_mgr_queue(struct ecrnx_cmd_mgr *cmd_mgr, struct ecrnx_cmd *cmd)
{
    ...
    ecrnx_printk_bh("%s-%d lock; \n", __func__, __LINE__);
    spin_lock_bh(&cmd_mgr->lock);

    if (cmd_mgr->state == ECRNX_CMD_MGR_STATE_CRASHED) {
        ecrnx_printk_err(KERN_CRIT"cmd queue crashed\n");
        cmd->result = -EPIPE;
        spin_unlock_bh(&cmd_mgr->lock);
        ecrnx_printk_bh("%s-%d unlock; \n", __func__, __LINE__);
        return -EPIPE;
    }
    ...
    if (!list_empty(&cmd_mgr->cmds)) {
        ...
        if (cmd_mgr->queue_sz == cmd_mgr->max_queue_sz) {
            ecrnx_printk_err(KERN_CRIT"Too many cmds (%d) already queued\n",
    ...
    if (!(cmd->flags & ECRNX_CMD_FLAG_NONBLOCK)) {
        ...
        if (!wait_for_completion_killable_timeout(&cmd->complete, tout)) {
            ecrnx_printk_always("cmd timed-out queue_sz:%d\n",cmd_mgr->queue_sz);
            cmd_dump(cmd);
            spin_lock_bh(&cmd_mgr->lock);
            ...
            if (!(cmd->flags & ECRNX_CMD_FLAG_DONE)) {
                ...
                cmd_complete(cmd_mgr, cmd);
                ecrnx_set_ipc_env_msg_flag(ecrnx_hw);
            }
            spin_unlock_bh(&cmd_mgr->lock);
        } else {
            if (!(cmd->flags & ECRNX_CMD_FLAG_DONE)) {
                ecrnx_printk_err("cmd error, not done, not timeout by error\n");
                cmd->result = -ETIMEDOUT;
                spin_lock_bh(&cmd_mgr->lock);
                cmd_complete(cmd_mgr, cmd);
                ecrnx_set_ipc_env_msg_flag(ecrnx_hw);
...
static int cmd_mgr_llind(struct ecrnx_cmd_mgr *cmd_mgr, struct ecrnx_cmd *cmd)
{
    ...
    if (!acked) {
        ecrnx_printk_err(KERN_CRIT "Error: acked cmd not found\n");
    } else {
        cmd->flags &= ~ECRNX_CMD_FLAG_WAIT_ACK;
        if (ECRNX_CMD_WAIT_COMPLETE(cmd->flags))
            cmd_complete(cmd_mgr, cmd);
...
static void cmd_mgr_print(struct ecrnx_cmd_mgr *cmd_mgr)
{
    ...
    spin_lock_bh(&cmd_mgr->lock);
    ecrnx_printk_always("q_sz/max: %2d / %2d - next tkn: %d\n",
